...
$code_val  = $subject['code'];
$title_val = $subject['title'];
$unit_val  = $subject['unit'];
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    } elseif ($subjectModel->codeExists($code_val, $id)) {
        $error = "Subject code '" . htmlspecialchars($code_val) . "' already exists!";
    } else {
        $updated_by = (int)($_SESSION['user_id'] ?? 0);

        if ($subjectModel->update($id, $code_val, $title_val, $unit_int, $updated_by)) {
